...
type Job struct {
	...
	CompletedAt       *time.Time             `json:"completedAt"`                                            // the timestamp when the worker finished the job
	...
	CurrentWorkerUID  *uuid.UUID             `json:"currentWorkerUID" gorm:"type:varchar(36); default:null"` // the uuid of the the worker bot that is currently working in this job
	WorkerInstance    string                 `json:"workerInstance" gorm:"index:worker_instance;"`           // the type of worker that should work on this job
